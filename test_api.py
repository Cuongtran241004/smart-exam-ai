...
def test_students():
    ...
    try:
        response = requests.get("http://localhost:7860/students")
        print("\nStudents Endpoint:")
        print(f"Status Code: {response.status_code}")
        print(f"Response: {response.json()}")
        return response.status_code == 200
...
def test_analyze_frame():
    ...
    try:
        ...
        response = requests.post("http://localhost:7860/analyze_frame", files=files)
        ...
        response = requests.post("http://localhost:7860/analyze_frame_base64", json=data)
